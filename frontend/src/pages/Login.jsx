...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMsg(err.message || 'Có lỗi xảy ra khi đăng nhập. Vui lòng thử lại.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGoogleLogin = () => {
    // Boilerplate flow for Google OAuth redir/call
    alert('Khung đăng nhập Google: Chức năng sẽ được tích hợp chính thức ở các bước sau.');
  };
          ...
          <div className="bg-white px-6 pb-8 pt-4 rounded-t-[36px] shadow-[0_-12px_24px_rgba(0,0,0,0.03)] flex-1 flex flex-col justify-between border-t border-slate-100">
            <div>
              ...
              <div className="text-center mb-6">
                ...
                <p className="text-sm text-slate-500 mt-1">Đăng nhập vào tài khoản UrbanCart của bạn</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled={loading}
                    className="w-full pl-11 pr-4 py-3.5 bg-slate-50 text-slate-800 placeholder-slate-400 rounded-full border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-900 transition-all text-sm font-medium"
                  />
                </div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled={loading}
                    className="w-full pl-11 pr-12 py-3.5 bg-slate-50 text-slate-800 placeholder-slate-400 rounded-full border border-slate-200 focus:outline-none focus:ring-2 focus:ring-sky-500/20 focus:border-sky-900 transition-all text-sm font-medium"
                  />
                  <button
                    type="button"
                ...
                <div className="text-right">
                  <a href="#forgot" className="text-xs font-semibold text-sky-900 hover:underline">
                    Quên mật khẩu?
                  </a>
                </div>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full py-3.5 bg-sky-900 text-white font-bold rounded-full shadow-lg shadow-sky-900/20 hover:bg-sky-950 active:scale-[0.99] transition-all duration-150 flex items-center justify-center gap-2 text-sm tracking-wide mt-2"
                >
                  {loading ? (
              ...
              <div className="flex items-center my-6">
                ...
                <span className="px-3 text-[10px] font-bold text-slate-400 tracking-wider">HOẶC ĐĂNG NHẬP VỚI</span>
                <div className="flex-grow border-t border-slate-200"></div>
              </div>
            ...
            <div className="text-center text-xs font-semibold text-slate-500 mt-6">
              Chưa có tài khoản?{' '}
              <Link to="/register" className="text-sky-900 hover:underline">
                Đăng ký ngay!
              </Link>
            </div>
